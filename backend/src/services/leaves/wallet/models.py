...
Now integrated directly into the leaves service.
"""
...
class EmployeeWallet(Base):
    ...
    vacation_accrued: Mapped[Decimal] = mapped_column(Numeric(5, 2), default=0)        # Maturate YTD
    ...
    legal_minimum_required: Mapped[Decimal] = mapped_column(Numeric(5, 2), default=20)
...
class WalletTransaction(Base):
    ...
    wallet_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("leaves.employee_wallets.id", ondelete="CASCADE"),
        nullable=False,
    )
